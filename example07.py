...
class MovementSystem(System):
    ...
    def on_update(self, ts, entity: Entity, components):
        ...
        if movement.selected:
            ...
            if glfw.get_key(Application().get_window().get_handle(), glfw.KEY_SPACE) == glfw.PRESS:
...
def main():
    ...
    # Create Enroll entities to registry
    new_entity1 = new_scene.enroll_entity()
    new_entity2 = new_scene.enroll_entity()

    # Register components to entity1
    new_scene.add_component(new_entity1, InfoComponent("e1"))
    new_scene.add_component(new_entity1, TransformComponent(utils.vec(0, 0, 0), utils.vec(0, 0, 0), utils.vec(1, 1, 1)))
    new_scene.add_component(new_entity1, LinkComponent(None))
    new_scene.add_component(new_entity1, RenderComponent([vertices, texture_coords], None))
    new_scene.add_component(new_entity1, MaterialComponent('M_Red_Textured'))
    new_scene.add_component(new_entity1, MovementComponent())

    # Register components to entity2
    new_scene.add_component(new_entity2, InfoComponent("e2"))
    new_scene.add_component(new_entity2, TransformComponent(utils.vec(2, 0, 0), utils.vec(0, 0, 0), utils.vec(1, 1, 1)))
    new_scene.add_component(new_entity2, LinkComponent(new_entity1))
    new_scene.add_component(new_entity2, RenderComponent([vertices], None))
    new_scene.add_component(new_entity2, MaterialComponent('M_Red_Simple'))
    new_scene.add_component(new_entity2, MovementComponent())

    SceneManager().add_scene(new_scene)

    # Create Register systems
    new_scene.register_system(TransformSystem([TransformComponent]))
    new_scene.register_system(LinkSystem([LinkComponent, TransformComponent]))
    new_scene.register_system(RenderingSystem([RenderComponent, MaterialComponent, TransformComponent]))
    new_scene.register_system(MovementSystem([MovementComponent, TransformComponent, InfoComponent]))
